...
def globaldata():
    ...
    # 保存归一化后的数据到新的CSV文件
    output_csv_path = "./scaler_sr_fits提取_未筛选.csv"  # 设置输出文件的路径
...
def duiqiOneandMPLus(JianceType):
    ...
    original_df = pd.read_csv("./scaler_sr_fits提取_未筛选.csv")
    ...
    # 将第二个CSV文件的 `noaaID` 列设置为索引，以便按 `noaaID` 进行匹配，因为前期已经sr_completed取交集了
...
if __name__ == '__main__':
    ...
    # globaldata()
    # print("=============================================================测试数据集========================================================")
    # mainTestdataFile(["LLM_VIT"],"./scaler_sr_fits提取_未筛选.csv","TSS")
    # mainTestdataFile(["LLM_VIT"],"./scaler_sr_fits提取_未筛选.csv","BSS")
