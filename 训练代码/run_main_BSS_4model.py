...
if __name__ == "__main__":
    ...
    #     args.embed_dim=item
    #     for _modelType in ["LLM_VIT","VIT","LLM","LSTM"]:
    #     for _modelType in ["VIT","NN"]:
        for _modelType in ["VIT"]:
            ...
            with open(results_filepath, 'w') as results_file:
                ...
                for count1 in range(args.num_dataset + 1):  # 循环处理0-9个数据集
                    ...
                    lr = args.lr
                    model_train = LLM_VIT.Model(args).float()
                    ...
                    for epoch in range(1, args.epochs + 1):
                        ...
                        val_metrics, all_predictions_y_true, all_predictions_y_prob = evaluate(validate_x, validate_y, model_train)  # 验证当前模型
                        ...
                        BS, thisBSS = BSS_eval_np(y_true, y_prob)
                        ...
                        print(f"====本轮验证集==========")
                        print(f"目前最佳BSS：{best_BSS}")
                        print(f"本轮验证集BSS：{thisBSS}")
                        if thisBSS > best_BSS:
                            best_BSS = thisBSS
                    ...
                    print(f"数据集 {count} 测试集BSS:", BSS)
                    print(f"数据集 {count} 验证集最优BSS:", best_BSS)

                    # 写入结果到文件
                    results_file.write(f"数据集 {count} 测试集BSS: {BSS}\n")
                    results_file.write(f"数据集 {count} 验证集最优BSS: {best_BSS}\n")
                    results_file.write("=================================================================\n")
                ...
                # new_row = [f'{model_base}-{commment}', data_TSS.mean(axis=0)[1]]   #data_BSS.append([BS, BSS])
                new_row = [f'{model_base}-{commment}', data_BSS.mean(axis=0)[1]] #取平均后拿到1的下表对应BSS
