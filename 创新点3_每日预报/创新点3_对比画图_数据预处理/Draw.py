...
def drawplot(isslect, directiondataset,offsets):
    csv_paths = [
        f"../对比{directiondataset}_{isslect}/threshold_tss_data_one_LLM_VIT_TSS.csv",
        f"../对比{directiondataset}_{isslect}/threshold_tss_data_MPlus_LLM_VIT_TSS.csv",
    ...
    if isslect == "混合":
        ...
        ARin = "ARs"
    plt.title(f'TSS curves of the LLMVIT and {indirectiondataset} on {inisslect} {ARin} in daily mode')
    plt.xlabel('Threshold')
    plt.ylabel('TSS')
    plt.legend()
    plt.grid(True)
...
if __name__ == '__main__':
    # 24 相同
    drawplot("单", "SR",[(0, 0), (0, 0)])
    drawplot("混合", "SR",[(0, 0), (0, 0)])
    drawplot("多", "SR",[(0, 0), (0, 0)])
    drawplot("单", "CCMC",[(0, 0), (0, 0)])
    drawplot("混合", "CCMC",[(0, 0), (0, 0)])
    drawplot("多", "CCMC",[(0, 0), (0, 0)])
    pass
