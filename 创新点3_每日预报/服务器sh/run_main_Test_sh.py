...
def evaluate(data_x, data_y, model, batch_size, args):
    model.eval()
    test_loss = 0
    correct = 0
    all_targets = []
...
def PreprocessNN(train_csv_path, validate_csv_path, test_csv_path):
    global FIRST
...
def mainTestdataDir(_modelTypeList, datapath, JianceType):
    ...
    parser.add_argument('--datasetname', type=str, default=f"{datapath}", help='new_data_scaler,data')
    ...
    for _modelType in _modelTypeList:
        ...
        with open(results_filepath, 'w') as results_file:
            ...
            for count1 in range(args.num_dataset + 1):  # 循环处理0-9个数据集
                ...
                train_csv_path = rf"{datapath}/{count}Train.csv"
                validate_csv_path = rf"{datapath}/{count}Val.csv"
                test_csv_path = rf"{datapath}/{count}Test.csv"
                if _modelType != "NN":
                    train_x, train_y, validate_x, validate_y, test_x, test_y, class_weight = Preprocess(train_csv_path,
                                                                                                        validate_csv_path,
                                                                                                        test_csv_path)
                elif _modelType == "NN":
                    train_x, train_y, validate_x, validate_y, test_x, test_y, class_weight = PreprocessNN(
                        train_csv_path,
                        validate_csv_path,
                        test_csv_path)
                model_filename = rf"./" \
                                 + f"/{JianceType}_{args.model_type}" + f"/model_{count}.pt"
                # 加载最佳模型
                model_test = torch.load(model_filename)
                # 测试集评估
                print(f"====================数据集{count}测试集轮评估数据=============================================")
                test_metrics, all_predictions_y_true, all_predictions_y_prob = evaluate(test_x, test_y, model_test,
                                                                                        batch_size, args)  # 使用测试集进行评估
                ...
                path = fr"{JianceType}_{args.model_type}_{count}"
                save_csv(y_pred, y_true, fr"./{JianceType}CSV/", path)
                # exit()
            ...
            filename = '创新点整理版本/创新点1_未筛选/result.xlsx'
            new_row = []
            if JianceType == "TSS":
                new_row = [f'{model_base}', data_TSS.mean(axis=0)[1]]
            elif JianceType == "BSS":
                new_row = [f'{model_base}', data_BSS.mean(axis=0)[1]]
            # 判断文件是否存在
...
def mainTestdataFile(_modelTypeList, datapath, JianceType):
    parser = argparse.ArgumentParser(description='Time-LLM')
    ...
    for _modelType in _modelTypeList:
        ...
        with open(results_filepath, 'w') as results_file:
            ...
            for count1 in range(args.num_dataset + 1):  # 循环处理0-9个数据集
                ...
                # dataname="new_data_scaler"
                dataname = args.datasetname
                train_csv_path = rf"{datapath}"
                validate_csv_path = rf"{datapath}"
                test_csv_path = rf"{datapath}"
                if _modelType != "NN":
                    train_x, train_y, validate_x, validate_y, test_x, test_y, class_weight = Preprocess(train_csv_path,
                                                                                                        validate_csv_path,
                                                                                                        test_csv_path)
...
def mainTestdataData(_modelTypeList, data, JianceType):
    parser = argparse.ArgumentParser(description='Time-LLM')
    ...
    parser.add_argument('--batch_size', type=int, default=16, help='batch size of train input data')
    parser.add_argument('--model_type', type=str, default='VIT', help='VIT,LLM_VIT')
    parser.add_argument('--datasetname', type=str, default=f"", help='new_data_scaler,data')
    parser.add_argument('--seed', type=int, default=2021, help='random seed')
    args = parser.parse_args()
    batch_size = args.batch_size
        ...
        with open(results_filepath, 'w') as results_file:
            ...
            for count1 in range(args.num_dataset + 1):  # 循环处理0-9个数据集
                ...
                if _modelType != "NN":
                    ...
                    # train_x, train_y, validate_x, validate_y, test_x, test_y, class_weight = PreprocessNN(train_csv_path,validate_csv_path,test_csv_path)
                    pass
                model_filename = rf"." \
                                 + f"\{JianceType}_{args.model_type}" + f"\model_{count}.pt"
...
if __name__ == "__main__":
    mainTestdataFile(["LLM_VIT"], "../../data/", "TSS")
