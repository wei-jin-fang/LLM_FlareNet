...
def duiqiOneandMPLus(JianceType):
    ...
    sc_waitcompare_path="../SC_waitcompare.csv"
    df2 = pd.read_csv(sc_waitcompare_path)

    # 确保字段名一致
    # 第一个表中的 T_REC 与 noaaID 和第二个表中的 prediction_date 与 noaaID 进行匹配
    df1 = df1[['predictday', 'noaaID', 'Class', 'one']]  # 提取必要的列
    df1.rename(columns={'predictday': 'T_REC'}, inplace=True)
    df2 = df2[['prediction_date', 'noaaID', 'MPlus']]  # 提取必要的列
