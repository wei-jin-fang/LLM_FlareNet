...
def LIME_analysis(data_x, data_y, model, count, model_typ):
    # 确保输入数据为 NumPy 数组
    ...
    explainer = lime.lime_tabular.LimeTabularExplainer(
        training_data=data_x_mean.values,
        feature_names=feature_names,
        class_names=["NC", "MX"],  # 假设二分类，调整为您的实际类别
        mode="classification"
    )

    # 选择要解释的样本（例如前几个样本）
    num_samples = min(5, len(data_x_mean))  # 限制样本数以减少计算量
    lime_explanations = []

    print("Computing LIME explanations for selected samples...")

    # 为每个样本生成 LIME 解释
    for idx in range(num_samples):
        exp = explainer.explain_instance(
            data_row=data_x_mean.iloc[idx].values,
            predict_fn=model_predict_proba,
            num_features=len(feature_names),
            labels=(1,)  # 仅解释正类（与 SHAP 一致）
        )
    ...
    for exp in lime_explanations:
        for feature_idx, weight in exp.local_exp[1]:  # 正类的解释
            lime_importance[feature_idx] += weight
    lime_importance /= num_samples  # 平均
    ...
    # 绘图类型
    plot_types = ["bar", "dot", "interaction", "decision"]
    ...
    for plot_type in plot_types:
        ...
        if plot_type == "bar":
            # 条形图
            plt.barh(lime_importance_df["Feature"], lime_importance_df["Importance"], color="#1f77b4")
            plt.xlabel("Mean LIME Importance")
            plt.title(f"LIME Feature Importance (Bar) - Model {count}")

        elif plot_type == "dot":
            # 点图
            plt.scatter(lime_importance_df["Importance"], lime_importance_df["Feature"],
                        color="#ff7f0e", s=100)
            plt.axvline(x=0, color="gray", linestyle="--")
            plt.xlabel("Mean LIME Importance")
            plt.title(f"LIME Feature Importance (Dot) - Model {count}")

        elif plot_type == "interaction":
            # 交互图（展示两个特征的联合重要性）
            # 这里我们选择两个特征（例如 R_VALUE 和 AREA_ACR）进行展示
            feature_pair = ["R_VALUE", "AREA_ACR"]
            pair_idx = [feature_names.index(f) for f in feature_pair]
            interaction_values = np.zeros((num_samples, len(pair_idx)))
            for idx, exp in enumerate(lime_explanations):
                for f_idx, weight in exp.local_exp[1]:
                    if f_idx in pair_idx:
                        interaction_values[idx, pair_idx.index(f_idx)] = weight
            plt.scatter(interaction_values[:, 0], interaction_values[:, 1],
                        c=data_y[:num_samples], cmap="coolwarm", s=100)
            plt.xlabel(f"{feature_pair[0]} Importance")
            plt.ylabel(f"{feature_pair[1]} Importance")
            plt.title(f"LIME Interaction Plot ({feature_pair[0]} vs {feature_pair[1]}) - Model {count}")
            plt.colorbar(label="Class")

        elif plot_type == "decision":
            # 决策图（展示单个样本的 LIME 解释）
            exp = lime_explanations[0]  # 取第一个样本
            exp.as_pyplot_figure(label=1)
            plt.title(f"LIME Decision Plot for Sample 0 - Model {count}")

        # 保存图形
        filename = f"LIME-{plot_type}-{model_typ}-positive_class-1-{count}.png"
        plt.savefig(filename, bbox_inches="tight", dpi=300)
        print(f"Saved LIME {plot_type} plot to {filename}")
        plt.close()
    ...
    # for _modelType in ["LLM_VIT"]:
        ...
        with open(results_filepath, 'w') as results_file:
            ...
            for count1 in range(args.num_dataset + 1):  # 循环处理0-9个数据集
                ...
                LIME_analysis(test_x, test_y,model_test,count,args.model_type)  # 使用测试集进行评估
                exit()
        #         continue
        #         del model_test
        #         # 清理 GPU 内存
