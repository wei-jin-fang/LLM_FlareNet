...
def SHAP(data_x, data_y, model, count, model_typ):
    ...
    '''
            补充：加载测试集
                    ...
                    LIME也找找
                        ...
                        最新版决策图
    '''
    plot_type = "heatmap"  # dot点图 bar条形图
    # 确保SHAP值是NumPy数组
    shap_values_positive_mean = np.array(shap_values_positive_mean)

    # 使用SHAP的heatmap绘制
    shap_values_obj = shap.Explanation(
        values=shap_values_positive_mean,
    ...
    # 设置标题
    plt.title("SHAP Values Heatmap for Positive Class")
    # 保存图形
