...
def SHAP(train_x,data_x, data_y, model, count, model_typ):
    ...
    '''
            补充：加载测试集
                    ...
                    LIME也找找
                        ...
                        最新版决策图
    '''
    plot_type = "bar"  # dot点图 bar条形图
    if plot_type == "bar":
        shap.summary_plot(
            shap_values_positive_mean,
            data_x_mean,
    ...
    # 保存图形
    filename = fr"../LLMVIT论文-shap图/求和版本/{plot_type}-{model_typ}-positive_class-{classnumber}-summary_plot-{count}.png"
    ...
    # for _modelType in ["LLM_VIT"]:
        ...
        with open(results_filepath, 'w') as results_file:
            ...
            for count1 in range(args.num_dataset + 1):  # 循环处理0-9个数据集
                ...
                SHAP(train_x,test_x, test_y,model_test,count,args.model_type)  # 使用测试集进行评估
                exit()
