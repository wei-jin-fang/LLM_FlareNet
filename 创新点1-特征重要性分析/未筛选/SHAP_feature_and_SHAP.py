...
def SHAPwithColor(train_x, data_x, data_y, model, count, model_typ):
    # https://grok.com/share/bGVnYWN5_1cf6c2f1-7ff5-44f7-87fb-5ccb8cac6e41
    # 确保输入数据为 NumPy 数组
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    feature_index = 9  # 用于 x 轴和 SHAP 值的特征 6是area 9是shg45
    ...
    # 保存图像
    plt.savefig(f'./shap_vs_{feature_names[feature_index]}_colored_by_{feature_names[color_feature_index]}.png')
    plt.close()
    ...
    # for _modelType in ["LLM_VIT"]:
        ...
        with open(results_filepath, 'w') as results_file:
            ...
            for count1 in range(args.num_dataset + 1):  # 循环处理0-9个数据集
                ...
                SHAPwithColor(train_x,test_x, test_y,model_test,count,args.model_type)  # 使用测试集进行评估
                exit()
