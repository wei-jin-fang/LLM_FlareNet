...
def SHAP(train_x,data_x, data_y, model, count, model_typ):

    # https://grok.com/share/bGVnYWN5_1cf6c2f1-7ff5-44f7-87fb-5ccb8cac6e41
    # 确保输入数据为 NumPy 数组


    # data_x:输入是n*Timestep*feature
    # 创建特征名称，仅包含 10 个特征
    ...
    # 选择整个数据集作为 SHAP 背景数据,换成trainx作为背景
    ...
    classnumber = 1
    shap_values_positive = shap_values[classnumber]  # 形状 (175, 40, 10)

    # 展平输入数据，按时间步求均值
    hap_values_positive_mean = np.mean(shap_values_positive, axis=1)  # 形状 (175, 10)
    data_x_mean = np.mean(data_x, axis=1)  # 形状 (n_samples, 10)

    feature_index=8
    data_x = np.array(hap_values_positive_mean)# 形状 (175, 10)
    shap_values_positive = np.array(data_x_mean) # 形状 (175, 10)

    # feature_values = data_x[:, :, feature_index].flatten()
    feature_values = data_x[:, feature_index]
    # shap_values = shap_values_positive[:, :, feature_index].flatten()
    shap_values = shap_values_positive[:, feature_index]

    # Create scatter plot
    plt.figure(figsize=(10, 6))
    plt.scatter(feature_values, shap_values, alpha=0.5, color='blue')
    plt.xlabel(f'{feature_names[feature_index]} Value')
    plt.ylabel(f'SHAP Value for {feature_names[feature_index]}')
    plt.title(f'SHAP Values vs {feature_names[feature_index]}')
    plt.grid(True)

    # Save the plot
    plt.savefig(f'./shap_vs_{feature_names[feature_index]}_平均的shap_value.png')
    plt.close()
    ...
    shap_values_positive_mean = np.mean(shap_values_positive, axis=1)  # 形状 (175, 10)
...
def SHAPwithColor(train_x, data_x, data_y, model, count, model_typ,color_feature_index):
    # https://grok.com/share/bGVnYWN5_1cf6c2f1-7ff5-44f7-87fb-5ccb8cac6e41
    # 确保输入数据为 NumPy 数组
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    # 选择整个数据集作为 SHAP 背景数据,换成 train_x 作为背景
    background_data = train_x
    background_tensor = torch.tensor(background_data, dtype=torch.float32)
    ...
    # 展平输入数据，按时间步求均值
    shap_values_positive_mean = np.sum(shap_values_positive, axis=1)  # 形状 (175, 10)
    data_x_mean = np.sum(data_x, axis=1)  # 形状 (n_samples, 10)

    feature_index =  6 # 用于 x 轴和 SHAP 值的特征 6是area 9是shg45
    # color_feature_index = 8 # 用于颜色的特征（例如第8列） rvalue

    # 提取用于绘图的数据
    feature_values = data_x_mean[:, feature_index]  # x 轴：特征值
    shap_values = shap_values_positive_mean[:, feature_index]  # y 轴：SHAP 值
    color_values = data_x_mean[:, color_feature_index]  # 颜色：第3列特征值

    # 归一化颜色值以映射到颜色范围
    norm = plt.Normalize(min(color_values), max(color_values))
    cmap = plt.cm.RdBu  # 红到蓝的颜色映射，红色表示高值，蓝色表示低值

    # 创建散点图
    plt.figure(figsize=(10, 6))
    scatter = plt.scatter(feature_values, shap_values, c=color_values, cmap=cmap, norm=norm, alpha=0.5)

    # 添加颜色条
    plt.colorbar(scatter, label=f'{feature_names[color_feature_index]} Value')

    # 设置标签和标题
    plt.xlabel(f'{feature_names[feature_index]} Value')
    plt.ylabel(f'SHAP Value for {feature_names[feature_index]}')
    plt.title(f'SHAP Values vs {feature_names[feature_index]} (Colored by {feature_names[color_feature_index]})')
    plt.grid(True)

    # 保存图像
    plt.savefig(f'./发群/shap_vs_{feature_names[feature_index]}_colored_by_{feature_names[color_feature_index]}.png')
    plt.close()
        ...
        with open(results_filepath, 'w') as results_file:
            ...
            for count1 in range(args.num_dataset + 1):  # 循环处理0-9个数据集
                ...
                for index in range(10):
                    SHAPwithColor(train_x,test_x, test_y,model_test,count,args.model_type,index)  # 使用测试集进行评估
                exit()
