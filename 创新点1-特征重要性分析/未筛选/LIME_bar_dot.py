...
# LIME(test_x, test_y, model_test, count, args.model_type)  # 使用测试集进行评估
import numpy as np
import torch
import lime.lime_tabular
import matplotlib.pyplot as plt

def LIMEdis(data_x, data_y, model, count, output_file='lime_global_importance.png'):
    """
    使用LIME生成全局特征重要性解释并绘制条形图，针对10个特征在所有时间步的全局重要性（正类）。

    参数：
        data_x: 输入数据，形状为 (n_samples, 40, 10)
        data_y: 标签数据，形状为 (n_samples,)
        model: 训练好的模型（输入为(batch_size, 40, 10)，输出为logits）
        count: 用于计算全局重要性的样本数
        output_file: 保存全局特征重要性图的文件名
    """
    # 确保输入数据为NumPy数组
    if isinstance(data_x, torch.Tensor):
        data_x = data_x.cpu().numpy()
    ...
    # 为了LIME分析，构造一个代表性输入：取每个特征在40个时间步的均值，形状为 (n_samples, 10)
    data_x_reduced = np.mean(data_x, axis=1)  # 形状从 (n_samples, 40, 10) 变为 (n_samples, 10)

    # 创建LIME解释器
    explainer = lime.lime_tabular.LimeTabularExplainer(
        training_data=data_x_reduced,
        feature_names=feature_names,
        class_names=['Negative', 'Positive'],  # 假设二分类
        mode='classification',
        discretize_continuous=False  # 避免LIME对连续特征分箱，防止条件表达式
    )

    # 定义模型的预测函数（保持原始输入形状 (batch_size, 40, 10)）
    '''
            # # data 形状为 (batch_size, 10)，需扩展为 (batch_size, 40, 10)
        # data_expanded = np.repeat(data[:, np.newaxis, :], 40, axis=1)  # 重复时间步
        # data_tensor = torch.tensor(data_expanded, dtype=torch.float32)
    ...
    def model_predict_proba(data_rows):
        batch_size = data_rows.shape[0]
        indices = []
        for i in range(batch_size):
            diff = np.abs(data_x_reduced - data_rows[i])
            idx = np.argmin(np.mean(diff, axis=1))  # 找到最接近的样本
            indices.append(idx)
        # 通过索引获取原始时间序列数据
        samples = data_x[indices]  # 形状 (batch_size, 40, 10)
        # 将数据转换为PyTorch张量
        ...
        with torch.no_grad():
            output = model(data_tensor)
        # 将 log_softmax 转换为概率（softmax）
        probs = torch.exp(output)  # 使用 torch.exp 而不是 np.exp
    ...
    sample_count = len(data_x)  # 限制样本数
    positive_samples = 0  # 记录正类样本数

    print(f"Computing LIME explanations for up to {sample_count} samples...")

    # 对多个样本进行LIME解释（仅正类样本）
    for idx in range(sample_count):

            sample = data_x_reduced[idx]
            ...
            # 提取正类的特征重要性
            for feature, value in exp.as_list(label=1):
                feature_idx = feature_names.index(feature.strip())  # 去除可能的空格
                global_importance[feature_idx] += abs(value)  # 使用绝对值
            positive_samples += 1

    # 计算平均重要性
    if positive_samples > 0:
        global_importance /= positive_samples
    else:
        print("No positive class samples found, returning zero importance.")
        global_importance = np.zeros(len(feature_names))

    # 归一化重要性
    global_importance = global_importance / (np.sum(global_importance) + 1e-10)

    # 绘制全局特征重要性条形图
    plt.figure(figsize=(10, 6))
    plt.barh(feature_names, global_importance, color='skyblue')
    plt.xlabel('Mean Absolute LIME Importance')
    plt.title('LIME Global Feature Importance (Positive Class)')
    plt.gca().invert_yaxis()  # 翻转y轴，重要特征在顶部
    plt.tight_layout()
    ...
    print(f"Global feature importance plot saved as {output_file}")

    return global_importance


def LIME(data_x, data_y, model, count, model_type):

    """
    使用LIME生成单个样本的特征重要性解释并绘制条形图，针对10个特征在所有时间步的特征重要性（正类）。
    ...
    参数：
        ...
        model: 训练好的模型（输入为(batch_size, 40, 10)，输出为logits）
        sample_idx: 要分析的样本索引
        output_file: 保存特征重要性图的文件名
    返回：
        feature_importance: 单个样本的特征重要性数组
    """
    import numpy as np
    import torch
    import matplotlib.pyplot as plt
    ...
    explainer = lime_tabular.LimeTabularExplainer(
        training_data=data_x_reduced,
        #IME 的 LimeTabularExplainer 期望 training_data 是一个二维数组，形状为 (n_samples, n_features)，其中每一行是一个样本，每一列是一个特征。
        # 进行取平均了
        feature_names=feature_names,
        class_names=['Negative', 'Positive'],  # 假设二分类
        mode='classification',
    ...
    def model_predict_proba(data_rows):
        '''
        你的 model_predict_proba 函数的问题：
        在 model_predict_proba 中，你固定使用了 data_x[sample_idx]（即样本索引为 sample_idx 的数据，形状为 (40, 10)），而不是使用 LIME 传入的 data_rows（形状为 (5000, 10)）。
        这导致无论 LIME 传入多少合成样本，model_predict_proba 始终只对单个样本进行预测，返回的概率数组形状为 (1, num_classes)，而不是预期的 (5000, num_classes)。
        因此，LIME 在拟合 Ridge 模型时，尝试将 5000 个合成样本的特征与仅 1 个预测标签对齐，引发了错误。
        '''
        # 通过索引获取原始时间序列数据
        samples = data_x[sample_idx]  # 形状 ( 40, 10)
        # data_rows 形状为 (num_samples, 10)，需要扩展为 (num_samples, 40, 10)
        num_samples = data_rows.shape[0]
        # 将每个样本的 10 个特征复制 40 次，模拟时间序列
        '''
                #  这个地方不确定合理不
        '''
        samples = np.repeat(data_rows[:, np.newaxis, :], 40, axis=1)  # 形状为 (num_samples, 40, 10)
        ...
        if torch.cuda.is_available():
            ...
            model.cuda()
        with torch.no_grad():
            output = model(data_tensor)  # 假设模型输出形状为 (num_samples, num_classes)
        # 报错了
        '''
        要解决 LIME 报错，需在 model_predict_proba 中将对数概率转换为标准概率。以下是修改后的代码和详细说明。
        '''
        probs = (output)  # 使用统一的对数概率进行输出
    ...
    # 对指定样本进行LIME解释
    sample = data_x_reduced[sample_idx]
    exp = explainer.explain_instance(
        data_row=sample,
        predict_fn=model_predict_proba,
        num_features=len(feature_names),
    ...
    for feature, value in exp.as_list(label=1):
        feature_idx = feature_names.index(feature.strip())  # 去除可能的空格
        feature_importance[feature_idx] = value  # 直接使用原始值（可正可负）

    # 归一化重要性（基于绝对值以保持相对重要性）
    abs_sum = np.sum(np.abs(feature_importance)) + 1e-10
    feature_importance = feature_importance / abs_sum

    # 绘制特征重要性条形图
    plt.figure(figsize=(10, 6))
    colors = ['skyblue' if val >= 0 else 'salmon' for val in feature_importance]
    plt.barh(feature_names, feature_importance, color=colors)
    plt.xlabel('LIME Feature Importance (Normalized)')
    plt.title(f'LIME Feature Importance for Sample {sample_idx} of Dataset{count}(Positive Class)')
    plt.gca().invert_yaxis()  # 翻转y轴，重要特征在顶部
    plt.tight_layout()

    # 保存图表sample_idx
    plt.savefig(f"{model_type}-LIME-{sample_idx}-bar", bbox_inches='tight')
    plt.close()
    ...
    # for _modelType in ["LLM_VIT"]:
        ...
        with open(results_filepath, 'w') as results_file:
            ...
            for count1 in range(args.num_dataset + 1):  # 循环处理0-9个数据集
                ...
                #
                #     def forward(self, x):
                #         log_probs = self.base_model(x)  # 原模型输出 log_softmax
                #         return torch.exp(log_probs)  # 转换为概率
                #
                #
                # # 使用包裹后的模型
                # prob_model = ProbModel(model_test)
                # model_test = prob_model
                # exit()
                # 测试集评估
