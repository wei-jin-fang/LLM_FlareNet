...
def SHAP_all(train_x,data_x, data_y, model, count, model_typ):
    ...
    train_x = train_x
    background_tensor = torch.tensor(train_x, dtype=torch.float32)
    ...
    for noaaid_number in range(n_samples):
        ...
        shap_values_positive_mean = pd.DataFrame(shap_values_positive, columns=feature_names)

        # 输入数据按时间步取均值
        data_x_mean = pd.DataFrame(single_sample, columns=feature_names)
        ...
        # 保存图像
        filename = f"./NN/{plot_type}-{model_typ}-positive_class-{classnumber}-summary_plot-{count}({noaaid_number})-({nowprob}).png"
