...
def getTenFeature():
    ...
    output_csv_path = r'ccmc_fits提取_未筛选.csv'
...
def globaldata():
    ...
    # 读取新的CSV文件
    new_csv_path = "./ccmc_fits提取_未筛选.csv"  # 替换为新CSV文件的路径
    ...
    # 保存归一化后的数据到新的CSV文件
    output_csv_path = "../创新点2_对比ccmc_先平均无标准差_未筛选/scaler_ccmc_fits提取_未筛选.csv"  # 设置输出文件的路径
...
def duiqiOneandMPLus(JianceType,datasetnumber):
    ...
    original_df = pd.read_csv("../创新点2_对比ccmc_先平均无标准差_未筛选/scaler_ccmc_fits提取_未筛选.csv")
    ...
    # 遍历第一个CSV文件的每一行
    for index, row in ccmc_selected_data.iterrows():
        noaaID = row['noaaID']
        MPlus = row['MPlus']
...
def calculate_tss_statistics(tss_list,JianceType,ProName):
    ...
    '''
    tss_list 是一个二维列表，其中每一行代表一个模型在不同阈值下的值。
    如果 tss_list 的维度是 10 x N（表示 10 个模型和 N 个阈值），
    那么 np.transpose(tss_list) 会将其转置为 N x 10 的形式。这样，每一列就对应了所有模型在同一个阈值下的值
    '''
...
if __name__ == '__main__':
    ...
    # globaldata()
    print("=============================================================测试数据集========================================================")
    # mainTestdataFile(["LLM_VIT"],"../创新点2_对比ccmc_先平均无标准差_未筛选/scaler_ccmc_fits提取_未筛选.csv","TSS")
    # mainTestdataFile(["LLM_VIT"],"../创新点2_对比ccmc_先平均无标准差_未筛选/scaler_ccmc_fits提取_未筛选.csv","BSS")
