...
def getTssFromFile(path):
    ...
    # 计算预测标签
    predicted = (df['MPlus'] > threshold).astype(int)  # 如果'one'列的值大于阈值，则预测为1，否则为0
...
if __name__ == '__main__':
    for _modelType in ["LLM_VIT"]:
        ...
        # 设置图表的标题和轴标签
        plt.title('CCMC_TSS vs Threshold')
        ...
        # plt.show()
        # 保存图表到文件
        plt.savefig(fr'compare_CCMC_TSS_vs_Threshold_42个对齐.png', dpi=300)  # 指定分辨率为300 DPI
